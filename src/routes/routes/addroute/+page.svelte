  ...
  onMount(() => {
   ...
   // function handleRouteChanged(event) {
    ...
    async function handleAdd(event) {
        routeData.created_by = id.toString()
        event.preventDefault();
        console.log('inside add : ',routeData);
        console.log('json add : ',JSON.stringify(routeData));
        try {
...
<form on:submit={handleAdd} class="flex justify-center gap-10 mx-auto max-w-7xl px-2 py-8 sm:px-8 pt-10">
    ...
    <div class="w-full h-[600px] flex flex-col">
        ...
        <MapRoute on:startingPlaceChanged={handleStartingPlaceChanged} on:destinationChanged={handleDestinationChanged} on:routeChanged/>
    </div>
</form>
